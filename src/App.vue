<template>
  <div id="app">
    ...
    <main id="main">
      <div class="home-page mod">
        <div class="holder">
          <div v-if="httpError !== null">
            <p>{{httpError}}</p>
          </div>
          <div v-else>
            <form name="capital">
            ...
            <button type="button" class="cta" @click="handleFormSubmit">Submit</button>
...
export default {
  ...
  computed: {
    countryName() {
      return this.capitalCities.length
        ? this.capitalCities[this.randomNumber].name
        : "";
    },
  },
  methods: {
    ...
    handleValidInput(obj) {
      if (
        ...
        this.capitalCities[this.randomNumber].capital.toLowerCase()
      ) {
        this.finalResult = `Final result is ${this.capitalCities[
          this.randomNumber
        ].capital.toLowerCase()}`;
      }
    },
